...
contract ValidationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @dev Optional HeartbeatManager; when set, validationRequest can submit the request as HTX via submitHeartbeat.
    address private _heartbeatManager;
    ...
    struct ValidationRegistryStorage {
        ...
        // rawHTXHash => requestHash (for HeartbeatManager callback)
        mapping(bytes32 => bytes32) _rawHTXHashToRequestHash;
    }
    ...
    function getHeartbeatManager() external view returns (address) {
        return _heartbeatManager;
    }

    function setHeartbeatManager(address heartbeatManager_) external onlyOwner {
        _heartbeatManager = heartbeatManager_;
    }

    /// @param snapshotId If non-zero and HeartbeatManager is set, the validation request is also submitted as rawHTX via submitHeartbeat.
    function validationRequest(
        ...
        // Submit the same data to HeartbeatManager as rawHTX when configured and snapshotId provided
        if (_heartbeatManager != address(0) && snapshotId != 0) {
    ...
    /// @notice Callback from HeartbeatManager when a heartbeat round is finalized.
    ...
    function onHeartbeatFinalized(
        ...
        ValidationRegistryStorage storage $ = _getValidationRegistryStorage();
        bytes32 requestHash = $._rawHTXHashToRequestHash[rawHTXHash];
        if (requestHash == bytes32(0)) return;

        ValidationStatus storage s = $.validations[requestHash];
        if (s.validatorAddress == address(0) || s.hasResponse) return;

        s.response = response;
        s.responseHash = heartbeatKey;
        s.tag = response == 100 ? "valid" : (response == 0 ? "invalid" : "inconclusive");
        s.lastUpdate = block.timestamp;
        s.hasResponse = true;
