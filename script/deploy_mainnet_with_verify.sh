...
BASE_COMMITTEE_SIZE="${BASE_COMMITTEE_SIZE:-25}"
COMMITTEE_GROWTH_BPS="${COMMITTEE_GROWTH_BPS:-0}"
MAX_COMMITTEE_SIZE="${MAX_COMMITTEE_SIZE:-100}"
MAX_ESCALATIONS="${MAX_ESCALATIONS:-0}"
QUORUM_BPS="${QUORUM_BPS:-6700}"
VERIFICATION_BPS="${VERIFICATION_BPS:-6700}"
RESPONSE_WINDOW_SEC="${RESPONSE_WINDOW_SEC:-300}"
JAIL_DURATION_SEC="${JAIL_DURATION_SEC:-120}"
MAX_VOTE_BATCH="${MAX_VOTE_BATCH:-100}"
MIN_OPERATOR_STAKE="${MIN_OPERATOR_STAKE:-70000000000}"
NODE_VERSION="${NODE_VERSION:-0.9.0}"
UNSTAKE_DELAY_SEC="${UNSTAKE_DELAY_SEC:-604800}"
MIN_COMMITTEE_VP="${MIN_COMMITTEE_VP:-140000000000}"
MOCK_STAKE_MINT="${MOCK_STAKE_MINT:-1000000000000}"
MOCK_REWARD_MINT="${MOCK_REWARD_MINT:-100000000000}"
...
EMISSIONS_WEEKLY_UNITS="${EMISSIONS_WEEKLY_UNITS:-}"
EMISSIONS_WEEKS="${EMISSIONS_WEEKS:-}"
...
REWARD_MAX_PAYOUT_PER_FINALIZE="${REWARD_MAX_PAYOUT_PER_FINALIZE:-0}"
USE_NOOP_SLASHING="${USE_NOOP_SLASHING:-false}"

if [[ -z "$EMISSIONS_SCHEDULE" ]]; then
  if [[ -z "$EMISSIONS_WEEKLY_UNITS" || -z "$EMISSIONS_WEEKS" ]]; then
    echo "Missing emissions config. Set EMISSIONS_SCHEDULE or EMISSIONS_WEEKLY_UNITS+EMISSIONS_WEEKS." >&2
    exit 1
  fi
...
import math
weekly = int("${EMISSIONS_WEEKLY_UNITS:-0}")
...
if [[ "${SKIP_DEPLOY_L2_SUITE:-0}" != "1" ]]; then
  ...
  (
    ...
    USE_MOCK_TOKENS=false \
      ...
      MOCK_STAKE_MINT="$MOCK_STAKE_MINT" \
      MOCK_REWARD_MINT="$MOCK_REWARD_MINT" \
      BASE_COMMITTEE_SIZE="$BASE_COMMITTEE_SIZE" \
      COMMITTEE_GROWTH_BPS="$COMMITTEE_GROWTH_BPS" \
      MAX_COMMITTEE_SIZE="$MAX_COMMITTEE_SIZE" \
      MAX_ESCALATIONS="$MAX_ESCALATIONS" \
      QUORUM_BPS="$QUORUM_BPS" \
      VERIFICATION_BPS="$VERIFICATION_BPS" \
      RESPONSE_WINDOW_SEC="$RESPONSE_WINDOW_SEC" \
      JAIL_DURATION_SEC="$JAIL_DURATION_SEC" \
      MAX_VOTE_BATCH="$MAX_VOTE_BATCH" \
      MIN_OPERATOR_STAKE="$MIN_OPERATOR_STAKE" \
      NODE_VERSION="$NODE_VERSION" \
      UNSTAKE_DELAY_SEC="$UNSTAKE_DELAY_SEC" \
      MIN_COMMITTEE_VP="$MIN_COMMITTEE_VP" \
...
if [[ "${SKIP_SET_MINTER:-0}" != "1" ]]; then
  ...
  echo "Skipping L1 NIL minter update (SKIP_SET_MINTER=1)."
  echo "Submit this via the L1 NIL multisig when ready:"
  echo "  Target: $L1_NIL_ADDRESS"
  echo "  Value: 0"
  echo "  Data: $(cast calldata "setMinter(address,bool)" "$L1_EMISSIONS_ADDRESS" true)"
fi
...
(
  ...
  FOUNDRY_DISABLE_SOLC_DOWNLOAD=1 FOUNDRY_SOLC_VERSION="$SOLC_VERSION" FOUNDRY_SOLC_PATH="$SOLC_PATH" forge verify-contract \
    ...
    --constructor-args "$(cast abi-encode 'constructor(address,address,uint256)' \
      "$L2_NIL_ADDRESS" "$DEPLOYER" "$UNSTAKE_DELAY_SEC")"
  ...
  FOUNDRY_DISABLE_SOLC_DOWNLOAD=1 FOUNDRY_SOLC_VERSION="$SOLC_VERSION" FOUNDRY_SOLC_PATH="$SOLC_PATH" forge verify-contract \
    ...
    --constructor-args "$(cast abi-encode 'constructor(address,address,uint256,uint32)' \
      "$STAKING_ADDRESS" "$DEPLOYER" "$MIN_COMMITTEE_VP" "$MAX_COMMITTEE_SIZE")"
  ...
  FOUNDRY_DISABLE_SOLC_DOWNLOAD=1 FOUNDRY_SOLC_VERSION="$SOLC_VERSION" FOUNDRY_SOLC_PATH="$SOLC_PATH" forge verify-contract \
    ...
    --compiler-version "$SOLC_VERSION" \
    --constructor-args "$(cast abi-encode 'constructor(address,address,address,address,address,uint32,uint32,uint32,uint8,uint16,uint16,uint256,uint256,uint256,uint256)' \
      "$DEPLOYER" \
      "$STAKING_ADDRESS" \
      "$SELECTOR_ADDRESS" \
      "$STAKING_ADDRESS" \
    ...
    (
      ...
      if [[ -n "${EMISSIONS_ARGS_PATH:-}" ]]; then
        FOUNDRY_DISABLE_SOLC_DOWNLOAD=1 FOUNDRY_SOLC_VERSION="$SOLC_VERSION" FOUNDRY_SOLC_PATH="$SOLC_PATH" ETHERSCAN_API_VERSION="$ETHERSCAN_API_VERSION" forge verify-contract \
          ...
          --compiler-version "$SOLC_VERSION" \
          --constructor-args-path "$EMISSIONS_ARGS_PATH"
      else
        FOUNDRY_DISABLE_SOLC_DOWNLOAD=1 FOUNDRY_SOLC_VERSION="$SOLC_VERSION" FOUNDRY_SOLC_PATH="$SOLC_PATH" ETHERSCAN_API_VERSION="$ETHERSCAN_API_VERSION" forge verify-contract \
          --rpc-url "$L1_RPC_URL" \
          --chain "$L1_CHAIN" \
