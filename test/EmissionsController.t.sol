...
<<<<<<< New base: Restore payable bridge fees for epoch payouts
...
||||||| Common ancestor
    ...
    function test_mintAndBridgeNextEpoch_rejectsEther() public {
        vm.warp(controller.nextEpochReadyAt());
        (bool ok,) = address(controller).call{value: 1}(abi.encodeWithSignature("mintAndBridgeNextEpoch()"));
        assertTrue(!ok, "non-payable entrypoint must reject msg.value");
    }
...
>>>>>>> Current commit: Restore payable bridge fees for epoch payouts
>>>>>>> Current commit: Restore payable bridge fees for epoch payouts
}
